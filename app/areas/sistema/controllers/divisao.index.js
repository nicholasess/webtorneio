...
(function () {
    ...
    angular
        .module('app')
        .controller('divisao.index', DivisaoCtrl);

    DivisaoCtrl.$inject = ['$scope', '$location', 'DivisaoService', '$rootScope', 'filterFilter', '$route'];

    function DivisaoCtrl($scope, $location, DivisaoService, $rootScope, filterFilter, $route) {
        ...
        $scope.deletar = function(item){
          DivisaoService.Deletar(item).then(function(data){
              $route.reload();
          }, function(item){
